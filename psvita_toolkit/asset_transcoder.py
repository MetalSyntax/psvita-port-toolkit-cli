...
1. **Textures.** Pillow (this project's one hard dependency) can decode
   ...
   mip chain, directly loadable via `sceGxmTextureInitLinear()` with zero
   runtime decode cost. If a real GPU-texture-compression tool
   (`PVRTexToolCLI` or `compressonatorcli` -- both real, independently
   downloadable tools, never bundled) is found on `PATH`, it ALSO attempts
   a genuinely hardware-compressed `.pvr`/`.dds` as a bonus -- best-effort,
   because this toolkit can't pin an exact CLI flag set across every
   version of a third-party tool it doesn't ship. See
